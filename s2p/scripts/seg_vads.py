...
def get_parser():
    ...
    parser.add_argument(
        "--start_index",
        "-s",
        help="index of manifest file to start doing vad.",
        type=int,
        required=True,
    )
    parser.add_argument(
        "--end_index",
        "-e",
        help="index of manifest file to end doing vad.(exclusive)",
        type=int,
        required=True,
    )
...
def main():
    ...
    end_idx = min(args.end_idx, len(lines))
    print(f"start doing vads from index {start_idx} to {end_idx}")
    for fpath in tqdm(lines[start_idx:end_idx]):
