...
class W2lViterbiDecoderWithEmissions(W2lViterbiDecoder):
    ...
    def decode(self, emissions):
        ...
        res = []
        for b in range(B):
            vit_path_npy = viterbi_path[b].numpy()
            consecutive_filter = np.diff(vit_path_npy, prepend=[-1]) != 0
            blank_tok_filter = vit_path_npy != self.blank
            sil_tok_filter = vit_path_npy != self.silence
            combined_filter = consecutive_filter & blank_tok_filter & sil_tok_filter
            tokens = vit_path_npy[combined_filter]
            filtered_emissions = emissions[b][combined_filter]
            new_hypo = [{
        ...
        #     "emissions": emissions[b].detach().numpy()}]
        #     for b in range(B)
        # ]
...
def process_predictions(
    ...
    for i, hypo in enumerate(hypos[: min(len(hypos), cfg.nbest)]):
        if torch.is_tensor(hypo["tokens"]):
            ...
            special_token_filter = tokens >= tgt_dict.nspecial
            valid_emisions = hypo["emissions"][special_token_filter]
            tokens = tokens[special_token_filter]
            entropy = -(F.softmax(valid_emisions, dim=-1) * F.log_softmax(valid_emisions, dim=-1)).sum(dim=-1).numpy()
            # print(entropy)
            # print(entropy.shape)
...
def generate(cfg: UnsupGenerateConfig, models, saved_cfg, use_cuda):
    ...
    def build_generator(cfg: UnsupGenerateConfig):
        ...
        if w2l_decoder == DecoderType.VITERBI_MASKED:

            return W2lViterbiDecoderWithEmissions(cfg, task.target_dictionary)
    ...
    with progress_bar.build_progress_bar(cfg.fairseq.common, itr) as t:
        for i, sample in enumerate(t):
            ...
            for i, sample_id in enumerate(sample["id"].tolist()):
                ...
                entropy_ary.append(entropy)

            num_sentences += (
                sample["nsentences"] if "nsentences" in sample else sample["id"].numel()
            )
    
    entropy_out_fpath = os.path.join(cfg.results_path, f"{cfg.fairseq.dataset.gen_subset}_entropy.npy")
    print(entropy_out_fpath)
    np.save(entropy_out_fpath, np.array(entropy_ary, dtype="object"))
