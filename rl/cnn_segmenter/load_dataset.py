...
class ExtractedFeaturesDataset(FairseqDataset):
    def __init__(
        ...
        if aux_target_postfix is not None:
            assert aux_target_dir_path is not None
            aux_target_dir_path = os.path.join(aux_target_dir_path, split)
            if not os.path.exists(aux_target_dir_path+f".{aux_target_postfix}"):
                logger.info(f"auxaliry target for {split} missing")
            else:
                with open(aux_target_dir_path+f".{aux_target_postfix}", "r") as t_f:
                    self.aux_tgt = [
